  ...
  return (
    <main className="bg-gray-100">
      ...
      <section className="mx-auto max-w-6xl px-6 sm:px-8 pt-10 pb-16">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10 items-center">
          ...
          <div className="relative">
            {/* You can manually add an image here later */}
            <div className="w-full h-[320px] md:h-[420px] rounded-2xl shadow-lg bg-neutral-200" />
      ...
      <section className="mx-auto max-w-3xl px-6 sm:px-8 py-10">
        <h2 className="text-2xl font-semibold text-black mb-3">{t("waitlist.title")}</h2>
        <p className="text-neutral-700 mb-4">{t("waitlist.description")}</p>
        ...
        <form onSubmit={handleWaitlist} className="flex flex-col sm:flex-row gap-3">
          ...
          >
            {waitlistState.loading ? t("waitlist.sending") : t("waitlist.join_list")}
          </button>
        </form>

        {waitlistState.ok && (
          <p className="mt-2 text-green-700 text-sm">{t("waitlist.success")}</p>
        )}
        {waitlistState.error && (
          <p className="mt-2 text-red-600 text-sm">{t("waitlist.error")}</p>
        )}
      </section>
    </main>
  );
